...
window.onload = function () {
		...
		var eventCallback = {
			ui: function(data) {
				...
				if (config.showWeapons == true) { document.querySelector('#weapon').style.display = 'none'; }
			},
			...
			setStatus: function(data) {
				...
				if (data.isdead == true) {
					...
					for (i = 0; i < data.status.length; i++) {
						...
						if(document.querySelector('#'+data.status[i].name+' span') && data.status[i].name == 'stamina') {
							document.querySelector('#'+data.status[i].name+' span').style.width = statusValue+'%'; 
						}
						else if (document.querySelector('#'+data.status[i].name+' span')) { 
