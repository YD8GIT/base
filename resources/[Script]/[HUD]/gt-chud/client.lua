				...
				if IsControlJustPressed(0, 246) then
					...
					if (inVehicle) then
						...
						speed = GetEntitySpeed(vehicle)
							...
							enableCruise = true
							speed = carSpeed
							TriggerEvent('esx:showColoredNotification', "Cruise control actief aan" ..speed.."km/h", 148)
				SendNUIMessage({
						cruise = true,
					})else
					...
					SendNUIMessage({
						cruise = false,
						TriggerEvent('esx:showNotification', "Cruise control ~r~uitgeschakelt.")
					})
					end 
						else
							TriggerEvent('esx:showColoredNotification', "U moet rijden om deze actie uit te voeren.", 6)
						end
					end
				end
			end
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if car ~= 0 and (wasInCar or IsCar(car)) then
			...
			if IsControlJustReleased(0, 183) then ---config touche pour la ceinture 
				beltOn = not beltOn
				if beltOn then 
	...
	while true do
		...
		if(IsPedInAnyVehicle(Ped)) then
			...
			if PedCar then
				...
				-- Engine HP
				engingeHP = GetVehicleEngineHealth(PedCar)
				SendNUIMessage({
